...
class JodiFamilyScreen extends StatefulWidget {
  ...
  @override
  State<JodiFamilyScreen> createState() => _JodiFamilyScreenState();
}

class _JodiFamilyScreenState extends State<JodiFamilyScreen>
  ...
  // ─── Computed helpers ──────────────────────────────────────
  int get _childCount =>
      selectedJodiFamily != null ? jodiFamily[selectedJodiFamily]!.length : 0;
  ...
  Future<void> _createTransaction(double amount) async {
    ...
    await _transactionService.createTransaction(
      TransactionModel(
        ...
        pana: selectedJodiFamily ?? '',
        digit: '',
      ),
    );
  }

  // ─── Filtered jodi family keys ────────────────────────────────────
  List<String> get _filteredKeys {
    if (_searchQuery.isEmpty) return jodiFamily.keys.toList();
    return jodiFamily.keys.where((k) => k.contains(_searchQuery)).toList();
  }

  // ─── Actions ───────────────────────────────────────────────
  void _onContinue() {
    if (selectedJodiFamily == null) {
      _snack('Please select a jodi family', isError: true);
      return;
    }
    if (_perJodiAmount <= 0) {
  ...
  Future<void> _confirmOrder() async {
    ...
    try {
      ...
      if (selectedJodiFamily == null) {
        throw Exception('No jodi family selected');
      }
      ...
      final futures = jodiFamily[selectedJodiFamily]!
          .map(
      ...
      _snack(
        'Bids placed! Family $selectedJodiFamily · $_childCount × ₹${_perJodiAmount.toStringAsFixed(0)} = ₹${_totalAmount.toStringAsFixed(2)}',
        isError: false,
      );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: _buildAppBar(),
      bottomNavigationBar: selectedJodiFamily != null
          ? _buildBottomPanel()
          : null,
      body: CustomScrollView(
        ...
        slivers: [
          ...
          SliverToBoxAdapter(child: _buildJodiFamilySelector()),
            ...
            SliverPadding(
              ...
              sliver: SliverGrid(
                ...
                delegate: SliverChildBuilderDelegate((_, idx) {
                  final child = jodiFamily[selectedJodiFamily]![idx];
                  final active = _perJodiAmount > 0;
                ...
                }, childCount: jodiFamily[selectedJodiFamily]!.length),
              ),
            ),
          ] else ...[
            SliverFillRemaining(
  ...
  // ─── Jodi Family Selector ─────────────────────────────────────────
  Widget _buildJodiFamilySelector() {
        ...
        children: [
          ...
          TextField(
            ...
            decoration: InputDecoration(
              hintText: 'Search jodi family (e.g. 12, 23)',
              hintStyle: TextStyle(
          ...
          // Horizontally scrollable jodi family chips — 2 rows
          SizedBox(
            height: 108,
            child: keys.isEmpty
                ? Center(
                    child: Text(
                      'No jodi family found for "$_searchQuery"',
                ...
                : GridView.builder(
                    ...
                    itemBuilder: (_, i) {
                      final family = keys[i];
                      final sel = selectedJodiFamily == family;
                      final count = jodiFamily[family]!.length;
                      return GestureDetector(
                        onTap: isOpenSessionDisabled
                            ? null
                            : () => setState(() {
                                selectedJodiFamily = family;
                                showConfirmation = false;
                                _confirmCtrl.reverse();
                              }),
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      '₹${_perJodiAmount.toStringAsFixed(0)} will be placed on all $_childCount jodis in family $selectedJodiFamily',
  ...
  Widget _buildChildJodiGridHeader() {
    ...
    return Container(
      ...
      child: Row(
        children: [
          const Icon(Icons.grid_view_rounded, color: _C.gold, size: 15),
          const SizedBox(width: 8),
          Text(
            'Family $selectedJodiFamily — $_childCount Jodis',
            style: const TextStyle(
              color: _C.gold,
              fontSize: 13,
              fontWeight: FontWeight.bold,
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const Text(
            'Select a jodi family to place bids',
          ...
          Text(
            isOpenSessionDisabled
                ...
                : 'Search or scroll to find your jodi family',
            style: TextStyle(
          ...
          Container(
            ...
            child: Column(
              children: [
                ...
                _row('Session', 'Open'),
                const SizedBox(height: 7),
                _row('Jodi Family', selectedJodiFamily ?? '—'),
                const SizedBox(height: 7),
